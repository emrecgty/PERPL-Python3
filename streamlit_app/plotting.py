...
def draw_2d_scatter_plots(xyzcolour_values, dims, info, zoom): # xyz_values
    ...
    if zoom == 0:
        ...
        if dims == 3:
            ...
            # if info['short_names'] is True:
            #     filename = info['short_results_dir']+r'/'+fig_name
            draw_2col_2d_scatter_plot(xyzcolour_values, 'X (nm)', 
                                      'Z (nm)', axes=(0, 2))
    ...
    if zoom > 0: # FIX ZOOM!!!!!
        ...
        # if info['short_names'] is True:
        #     filename = info['short_results_dir']+r'/'+fig_name
        draw_2col_2d_scatter_plot(zoomed_xyzc, title, 'X (nm)', 'Y (nm)')
...
def draw_2col_2d_scatter_plot(xyzcolour_values,
    title, x_label, y_label, axes=(0, 1)
    ):
...
def plot_histograms(d_values, dims, filter_distance, binsize=1):
    ...
    plot_histogram(np.absolute(d_values[:, 3]), "xy", filter_distance, binsize=binsize)
    #print("xy d_values[:, 3][0]: ", d_values[:, 3][0])

    plot_histogram(np.absolute(d_values[:, 3]),
        "xy", filter_distance, standardise='2d', binsize=binsize
        )

    plot_histogram(np.absolute(d_values[:, 0]), "x", filter_distance, binsize=binsize)
    #print("x d_values[:, 0][0]: ", d_values[:, 0][0])
    plot_histogram(np.absolute(d_values[:, 1]), "y", filter_distance, binsize=binsize)


    if dims == 3:
        plot_histogram(np.absolute(d_values[:, 4]), "xz", filter_distance, binsize=binsize)
        plot_histogram(np.absolute(d_values[:, 5]), "yz", filter_distance, binsize=binsize)
        plot_histogram(np.absolute(d_values[:, 6]), "xyz", filter_distance, binsize=binsize)

        plot_histogram(np.absolute(d_values[:, 2]), "z", filter_distance, binsize=binsize)
...
def plot_histogram(data_values, data_description, filterdist, binsize=1, standardise=None):
